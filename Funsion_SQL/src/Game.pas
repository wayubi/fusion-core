...
{É`ÉÉÉbÉgÉãÅ[ÉÄã@î\í«â¡ÉRÉRÇ‹Ç≈}
{òIìXÉXÉLÉãí«â¡}
				if tc.VenderID <> 0 then continue; //òIìXíÜÇÃà⁄ìÆã÷é~
{òIìXÉXÉLÉãí«â¡ÉRÉRÇ‹Ç≈}
				RFIFOM1(2, xy);
				tc.NextFlag := true;
				tc.NextPoint := xy;
{ÉpÅ[ÉeÉBÅ[ã@î\í«â¡}
				//ìØàÍÉ}ÉbÉvì‡PTMÇ…èäç›ÇímÇÁÇπÇÈ
...
{èCê≥}                                          if (tc.JID <> 0) or ((DebugCMD and $0020) <> 0) then //ÉmÅ[ÉrÉXÇ©ÇÁÇÕèoóàÇ»Ç¢
						        ...
						        //êEã∆ïœçX
						        	...
						        	tc.JID := i;
											//ÉXÉeÅ[É^ÉXçƒåvéZ
											tc.ClothesColor := 0; //ã≠êßìIÇ…èâä˙íl
											//ÉIÉvÉVÉáÉìèâä˙âª
											if (tc.Option <> 0) then begin
												tc.Option := 0;
												//å©ÇΩñ⁄ïœçX
...
{èCê≥}
              ...
              if (tc.Skill[i].Data.Job[tc.JID]) then begin
						...
						for i := 210 to 336 do begin //äÿçëç˜à‰êÍóp
              if (tc.Skill[i].Data.Job[tc.JID]) then begin
							tc.Skill[i].Lv := tc.Skill[i].Data.MasterLV;
              end;
						end;
   ...
   Socket.SendBuf(buf, 15); 
					...
					2: //égópÉAÉCÉeÉÄ
						begin
							...
							2: //Â¢ÇÃâH
								begin
{ÉAÉWÉgã@î\í«â¡}                                                        if tc.item[w].Amount <= 0 then exit;
									i := MapInfo.IndexOf(tc.Map);
									j := -1;
									if (i <> -1) then begin
										mi := MapInfo.Objects[i] as MapTbl;
		...
		$00c8: // Send Request to Buy Item from NPC Shop
			begin
				...
				if tc.Zeny < l then begin
					//Ç®ã‡Ç™ë´ÇËÇ»Ç¢
					WFIFOW(0, $00ca);
					WFIFOB(2, 1);	//1=Ç®ã‡Ç™ë´ÇËÇ»Ç¢ 2=èdó ÉIÅ[ÉoÅ[ 3=ÉAÉCÉeÉÄç≈ëÂéÌóﬁêîÉIÅ[ÉoÅ[
					Socket.SendBuf(buf, 3);
					//èdó ÉIÅ[ÉoÅ[
					WFIFOW(0, $00ca);
					WFIFOB(2, 2);	//1=Ç®ã‡Ç™ë´ÇËÇ»Ç¢ 2=èdó ÉIÅ[ÉoÅ[ 3=ÉAÉCÉeÉÄç≈ëÂéÌóﬁêîÉIÅ[ÉoÅ[
					Socket.SendBuf(buf, 3);
...
{éÊà¯ã@î\í«â¡}
		...
		$00ef: //éÊà¯è≥ë¯
			begin
				...
				//ÉLÉÉÉâÇPèdó çXêVÉpÉP
				WFIFOW(0, $00b0);
				WFIFOW(2, $0018);
				WFIFOL(4, tc.Weight);
				tc.Socket.SendBuf(buf, 8);
				//ÉLÉÉÉâÇQèdó çXêVÉpÉP
				WFIFOL(4, tc1.Weight);
				tc1.Socket.SendBuf(buf, 8);
		...
		$00f9: //ÉpÅ[ÉeÉBÅ[çÏê¨
			begin
				...
				if (tc.Skill[1].Lv < 7) and (not DisableSkillLimit) then begin
					//ñ{óàÇ»ÇÁÉåÉxÉãïsë´ÇÃÉpÉPÇèoÇ∑ÅH
					WFIFOW( 0, $00fa);
					WFIFOB( 2, 1);         // 1:ìØñºÅ` 2:ä˘Ç…Å`
					Socket.SendBuf(buf, 3);
				end else begin
					if tc.PartyName <> '' then begin
						i := 2;
...
{NPCÉCÉxÉìÉgí«â¡ÉRÉRÇ‹Ç≈}
				if tm.gat[tc.Point.X][tc.Point.Y] = 0 then begin //à⁄ìÆïsî\èÍèäÇÕÉÅÉÇïsâ¬
					WFIFOW(0, $0189);
					WFIFOW(2, 1);
					Socket.SendBuf(buf, 4);
					continue;
...
{ÉJÅ[Égã@î\í«â¡ÉRÉRÇ‹Ç≈}
		//--------------------------------------------------------------------------
		$012a: //ëÈÅAÉyÉRÉyÉRÅAÉJÅ[ÉgÇäOÇ∑
			begin
      ...
      begin
		...
		$01af: //ÉJÅ[ÉgÉ`ÉFÉìÉW
			begin
				...
				RFIFOW(2, w);
				case w of
					1: tc.Option := (tc.Option and $F877) or $0008; //ÉJÅ[Ég1
					2: tc.Option := (tc.Option and $F877) or $0080; //ÉJÅ[Ég2
					3: tc.Option := (tc.Option and $F877) or $0100; //ÉJÅ[Ég3
					4: tc.Option := (tc.Option and $F877) or $0200; //ÉJÅ[Ég4
					5: tc.Option := (tc.Option and $F877) or $0400; //ÉJÅ[Ég5
				end;
				//å©ÇΩñ⁄ïœçX
				WFIFOW(0, $0119);
				WFIFOL(2, tc.ID);

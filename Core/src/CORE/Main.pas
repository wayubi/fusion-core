  ...
  if (ts.isSlave = false) then begin
	for k := 0 to 7 do begin
		...
		if ItemDropType then begin
			if Random(ItemDropDenominator) <= i then DropFlag := true; //èdóÕédólÅBÉäÉìÉSÇóéÇ∆Ç∑ÅB
		end else begin
			if Random(ItemDropDenominator) < i then DropFlag := true; //ñ{óàÇÃ(?)édólÅBÉäÉìÉSÇÕóéÇ∆Ç≥Ç»Ç¢ÅB
		end;
    ...
    if (j <> -1) then begin
      ...
      if (tg.GSkill[10000].Lv < 1) then begin
		...
		if not miss then begin
			...
			if ts.Stat1 = 2 then i := 21 //ìÄåãèàóù
			else i := ts.Element;
			dmg[0] := dmg[0] * ElementTable[AElement][i] div 100; //ëÆê´ëäê´ï‚ê≥
			if ts.Stat1 = 5 then dmg[0] := dmg[0] * 2; //ÉåÉbÉNÉX_ÉGÅ[ÉeÉãÉi
		end else begin
			//çUåÇÉ~ÉX
			dmg[0] := 0;
		end;
...
begin
	...
	with ts.Data do begin
		...
		if crit then dmg[5] := 10 else dmg[5] := 0; //ÉNÉäÉeÉBÉJÉãÉ`ÉFÉbÉN
		//VITÉ{Å[ÉiÉXÇ∆Ç©åvéZ
		j := ((tc.Param[2] div 2) + (tc.Param[2] * 3 div 10));
		k := ((tc.Param[2] div 2) + (tc.Param[2] * tc.Param[2] div 150 - 1));
		if j > k then k := j;
		if tc.Skill[33].Tick > Tick then begin //ÉGÉìÉWÉFÉâÉX
...
begin
	with tc do begin
		...
		if not miss then begin
			...
			if tc1.Stat1 = 2 then i := 21 //ìÄåãèàóù
			else i := 1;
			dmg[0] := dmg[0] * ElementTable[AElement][i] div 100; //ëÆê´ëäê´ï‚ê≥
			if tc1.Stat1 = 5 then dmg[0] := dmg[0] * 2; //ÉåÉbÉNÉX_ÉGÅ[ÉeÉãÉi
		end else begin
			//çUåÇÉ~ÉX
			dmg[0] := 0;
		end;
	...
	with ts do begin
		if tm.NPC.IndexOf(ts.ATarget) <> -1 then begin
			...
			if (abs(ts.Point.X - tn.Point.X) <= 1) and (abs(ts.Point.Y - tn.Point.Y) <= 1) then begin
				...
				//ÉAÉCÉeÉÄÇÕà⁄ìÆÇµÇ»Ç¢ÇÃÇ≈ÇªÇÃÇ‹Ç‹
				WFIFOW(0, $0088);
				WFIFOL(2, ts.ID);
				WFIFOW(6, ts.Point.X);
				WFIFOW(8, ts.Point.Y);
			...
			if (Option and 16 <> 0) and (Skill[129].Lv <> 0) and (Random(1000) < Param[5] * 10 div 3) then begin //ämó¶É`ÉFÉbÉN
				...
				if sl.Count <> 0 then begin
					for k1 := 0 to sl.Count - 1 do begin
						...
						dmg[0] := dmg[0] * ElementTable[0][ts1.Element] div 100; //ëÆê´ëäê´ï‚ê≥
...
{:119}
            ...
            if (ts.HP > 0) then begin
						...
						if (ts.Data.Race = 1) or (ts.Element mod 20 = 9) then begin
							...
							//âÒïúó  = (( BaseLv + INT) / 8ÇÃí[êîêÿéÃÇƒ ) * ( ÉqÅ[ÉãLv x 8 + 4 )
...
{í«â¡:119}
        ...
        5,42,46,253: //ÉoÉbÉVÉÖÅAÉÅÉ}Å[ÅADSÅAÉsÉAÅ[ÉXÅASB
					begin
						//É_ÉÅÅ[ÉWéZèo
						if (MSkill = 5) or (MSkill = 253) then begin
							DamageCalc3(tm, tc, tc1, Tick, 0, tl.Data1[MUseLV], tl.Element, tl.Data2[MUseLV]);
						...
						if (MSkill = 46) and (Weapon = 11) then begin //DSÇÕ2òAåÇ
							dmg[0] := dmg[0] * 2;
							j := 2;
						end else if MSkill = 56 then begin //ÉsÉAÅ[ÉXÇÕts.Data.Scale + 1âÒhit
							j := 1;
	...
	with ts do begin
		...
		for j := 1 to (Tick - MoveTick) div spd do begin
			...
			if ppos = pcnt then begin
				...
				//ÉpÉPëóêM(ã|çUåÇÇ≈ÇÃà íuÇ∏ÇÍëŒçÙ)
				WFIFOW(0, $0088);
				WFIFOL(2, ts.ID);
				WFIFOW(6, ts.Point.X);
				WFIFOW(8, ts.Point.Y);
...
//îΩåÇÉÇÅ[Éh
	with ts do begin
		...
		//çUåÇëŒè€Ç™éÄÇÒÇ≈Ç¢ÇÈÇ©ÉçÉOÉAÉEÉgÇµÇƒÇ¢ÇÈ
			ATarget := 0;
			ARangeFlag := false;
      ...
      MoveWait := Tick + 5000;
			...
			if Data.isDontMove then begin
				...
				if (not EnableMonsterKnockBack) or (DmgTick <= Tick) then begin
					...
					if pcnt = 0 then begin
						//ÉLÉÉÉâÇÃèäÇ‹Ç≈ï‡ÇØÇ»Ç¢=çUåÇïsî\ÅAÉ^Å[ÉQÉbÉgâèú
						//if j <> 100 then //DebugOut.Lines.Add('*** TraceError 2');
						//ATarget := 0;
						MoveWait := Tick + 5000;
					end else begin
						//ÉLÉÉÉâÇÃèäÇ‹Ç≈à⁄ìÆäJén
						ppos := 0;
						MoveTick := Tick;
						tgtPoint := xy;
